...
class Frame extends THREE.Group {
    constructor(parent, mobile, material) {
        ...
        const door_width = 30;
        const left_door = this.create_door(door_width, yPos * 2, [-door_width / 2, 0, 0], new Lock(parent, mobile, material));
        this.add(left_door);
    ...
    create_door(width, height, position, lock) {
        ...
        const baseGeo = new THREE.BoxGeometry(width, height, 3);
        ...
        const torus_padding = padding * 2.5;
        ...
        const torusGeometry = new THREE.TorusKnotGeometry(1, 0.5, 64, 16);
        const torus_nw = new THREE.Mesh(torusGeometry, this.material)
        ...
        if (lock) {
            if (position[0] > 0) {
                lock.position.set(-13, 0, 0)
            } else {
                lock.position.set(13, 0, 0)
            }


            lock.scale.multiplyScalar(0.5);
            group.add(lock);
        }
    ...
    update(timeStamp) {
        ...
        if (this.torii.length > 0) {
            for (let torus_group of this.torii) {
                ...
                torus_group.position.z = 1.2 + 0.3 * Math.sin(timeStamp / 800);
            }
        }
